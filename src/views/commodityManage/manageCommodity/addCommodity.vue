<template>
  <div class="root-container">
    ...
    <div class="from-group clearfix">
      ...
      <div class="form-item">
        <span>商品产地：</span>
        <el-select :disabled="isDisable" v-model="locality" placeholder="请选择">
          <el-option
            v-for="item in localitySelect"
            :key="item.label"
            :label="item.label"
            :value="item.label"
          ></el-option>
      ...
      <div class="form-item">
        ...
        >
          ...
          <div slot="file" slot-scope="{file}" class="cover-cont">
      ...
      <div class="form-item">
        <span>详情轮播顶图：</span>
        <el-upload
          action="http://192.168.3.88:8089/shop/uploadMultipleFiles"
          :auto-upload="false"
          name="files"
          list-type="picture-card"
...
export default {
  ...
  methods: {
    ...
    examineInfoComplete () {
      ...
      if (this.content.slice(3, -4).trim() === '') return errorInfo['8content']
      return true
    },
    ...
    // 提交按钮
    saveEnable () {
      var isEmpty = this.examineInfoComplete()
      if (isEmpty !== true) {
        ...
        returnImg: this.swiperImgArr
          .map(item => {
            if (item.response !== undefined) {
              return item.response.data[0].data
            } else {
              return item.url
            }
          })
