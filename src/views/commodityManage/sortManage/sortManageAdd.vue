<template>
  <div class="sortManageAdd root-container">
    ...
    <div class="from-group clearfix">
      <div class="form-item">
        <span>{{nameTitle}}：&nbsp;&nbsp;&nbsp;</span>
        <el-input :disabled="true" class="form-input" :value="this.$store.state.username"></el-input>
      </div>
      <div class="form-item">
        <span>父级分类：</span>
...
import { examineInfoComplete } from '../../../assets/js/common'
export default {
  data () {
    return {
      title: '新增分类',
  ...
  methods: {
    handleSub (status) {
      ...
      if (this.mode === 'update') {
        ...
        this.$http.sortManage.update(data).then(
          res => {
            this.$message({
              type: 'success',
              message: '修改成功'
      ...
      this.$http.sortManage.sortAdd(data).then(
        res => {
          this.$message({
            type: 'success',
            message: '添加成功'
