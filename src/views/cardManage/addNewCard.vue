    ...
    submitForm (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (this.mode === 'add') {
            cardModel.addCustomerCard(
              cardInfo,
              ({ data: { data } }) => {
                this.increaseCardPrompt('添加成功')
              },
              () => {
                this.increaseCardPrompt('添加失败，请检查字段是否正确')
              }
            )
          } else {
            this.submitUpdate(cardInfo)
          }
        } else {
          return false
        }
