  ...
  <div class="root-container activityManage">
    ...
    <listPanel class="grap" title="活动列表">
      <div>
        ...
        <el-table
          :data="searchContent.length === 0 ?tableData:tableSearchData"
          :style="{ width: '100%' }"
        ...
        >
          <el-table-column label="活动封面" width="180">
            <template slot-scope="scope">
              <img :style="{width: '100%',height: '130px'}" :src="scope.row.activityImg" alt />
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    // 根据页面获取数据
    getAll (page) {
      this.$http.activityManage.getAll(page).then(
        res => {
    ...
    handleToggle (index, item) {
      ...
      this.$http.activityManage.updateStatus(item.activityId, status).then(
        res => {
    ...
    handleDelete (index, item) {
      ...
      remove.call(this, () => {
        this.$http.activityManage.delete(item.activityId).then(
          res => {
